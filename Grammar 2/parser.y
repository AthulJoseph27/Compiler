...
%token <num> NUM
%token <id>VAR
%token RETURN IF FUN WRITE_VALUE READ_VALUE
%nonassoc IFX
%nonassoc ELSE
%left GE LE EE NE '>' '<' AND OR
%left '+' '-'
%left '*' '/'
%type <nPtr> Stmt E Stmts IfStmt IfElseStmt AsgStmt RetStmt

%start Program

%%

Program     : READ_VALUE'('VAR')'';' VAR'='FUN'('VAR')' '{' Stmts '}' WRITE_VALUE'('VAR')'';'       { generate_code($13); free_node($13); }
            ;
...
IfElseStmt  : IF'('E')''{'Stmts'}'ELSE'{'Stmts'}' { $$=add_node(IF,3,$3,$6,$10);}
            ;
RetStmt     : RETURN E';'             { $$ = add_node(RETURN,1, $2);}
            ;
E           : E '+' E                 { $$ = add_node('+',2,$1,$3);}
            | E '-' E                 { $$ = add_node('-',2,$1,$3);}
            | E '*' E                 { $$ = add_node('*',2,$1,$3);}
            ...
            | '('E')'                 { $$ = $2;}
            | NUM                     { $$ = create_node($1,NUM_TYPE); }
            | VAR                     { $$ = create_node(getId($1),VARIABLE_TYPE); }
            | FUN'('E')'              { $$ = add_node(FUN,1,$3);}
            ;
%%
